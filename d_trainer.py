...
class DiscriminatorTrainer(object):
    def __init__(self, config, train_data, valid_data, W_e_init, word2idx):
        ...
        self.train_data = np.asarray(train_data[0])
        self.valid_data = np.asarray(valid_data[0])
    ...
    def train(self):
        with self.sv.managed_session() as sess:
            ...
            train_dataset = Dataset(self.train_data, self.batch_size, self.pad_idx)
            valid_dataset = Dataset(self.valid_data, self.batch_size, self.pad_idx)
            train_generator = Batch_generator(train_dataset)
            valid_generator = Batch_generator(valid_dataset)
            ...
            while not self.sv.should_stop():
                ...
                pbar.update(1)
                train_data = train_generator.generate_d_pretrain_data_batch()
                train_label = train_generator.generate_d_pretrain_label_batch()
                ...
                if  step % self.log_step == 0: # default : 50
                    ...
                    if self.validation:
                        ...
                        valid_data = valid_generator.generate_d_pretrain_data_batch()
                        valid_label = valid_generator.generate_d_pretrain_label_batch()
